...
6. **👑 이광수식 지수 역행 주도주 스캐너 (Relative Strength Engine)**
   - 지수 급락일(-1% 이상) 속에서도 하락하지 않고 버티거나 **양봉 마감**하는 차기 주도주 발굴.
   - 거래량 폭발(20일 MA 대비) + AVWAP 지지선 유지 여부를 복합 평가하여 **주도주 점수(0~100점) & 등급(S/A/B)** 부여.
   - 5종목 이내 압축 투자 원칙 및 **매수가 대비 -10% 기계적 손절가(Trading Plan)** 자동 산출.
...
### 4. 👑 이광수식 지수 역행 주도주 스캐너

지수 급락/하락 압력을 이겨내고 **양봉 마감 및 거래량이 폭발하는 차기 주도주**를 상대 강도(RS) 점수와 함께 자동 선별합니다:

```bash
# [국내 우량주] KOSPI/KOSDAQ 대표 주도주 40종목 대상 스캔 (기본값)
python main.py -l

# [미국 빅테크] S&P500/나스닥 대표 빅테크 17종목 대상 스캔
python main.py -l --universe us

# [글로벌 통합] 국내 + 미국 대표 주도주 57종목 일괄 스캔
python main.py -l --universe all

# [내 관심그룹] watchlist.json에 등록된 그룹 대상 스캔
python main.py -l -g 보유종목

# [상세 필터] 최소 주도주 점수(예: 70점 이상 우량주만) 및 특정 벤치마크 지정
python main.py -l --min-score 70 --bench ^KS11
```

---

### 5. 보유/관심 종목 그룹별 스캐너 모드
