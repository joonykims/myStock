...
## [v2.0.2] - 2026-09-15
### 🛠️ 이광수식 주도주 스캐너 우측 상세 패널 열기/닫기 토글 추가
- **사이드 패널 접기/펼치기 기능 (`app.py`)**:
  - 좌측 테이블 상단에 "◀ 패널 닫기" / "▶ 패널 열기" 토글 버튼 추가, 닫으면 테이블이 전체 폭으로 확장
  - 우측 상세 패널 헤더에 "✖" 닫기 버튼 추가 (기존 `🔍 시장 수급 스캐너` 탭과 동일한 UX로 통일)
  - 상태는 `st.session_state["leader_peek_panel_open"]`에 저장되어 종목 재선택 시에도 유지
