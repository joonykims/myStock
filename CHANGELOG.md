...
## [v2.0.1] - 2026-09-15
### 🛠️ 웹 대시보드 플로팅 사이드 패널(Side Peek) 드래그 리사이징 및 레이아웃 개선
- **플로팅 사이드 드로어 크기 조절(Resizing) 기능 추가**:
  - 오버레이로 열린 차트 패널의 왼쪽 가장자리 선을 마우스로 드래그하여 패널 폭을 자유롭게 조절(확장/축소)할 수 있도록 자바스크립트 커스텀 이벤트 주입
  - 리액트 렌더링 충돌을 우회하기 위해 `!important` 속성을 동적으로 주입하여 부드러운 드래그 UX 보장
- **레이아웃 겹침 및 사이드바 증발 버그 완벽 수정**:
  - `st.container(border=True)`를 활용해 오버레이 타겟팅을 가장 가까운 `stVerticalBlockBorderWrapper`로 정확히 제한하여, 전체 메인 레이아웃이 쪼그라들거나 사이드바가 화면 밖으로 밀려나는 문제 해결
  - Streamlit 최상단 헤더 영역(`stHeader`)의 햄버거 메뉴(사이드바 열기 버튼)가 보존되도록 `display: none` 대신 불필요한 우측 툴바만 선별적으로 숨김 처리
  - 메인 본문의 `padding-top`을 `2.5rem`으로 조정하여 탭 라디오 버튼과 헤더가 겹치는 현상 방지

---

## [v2.0.0] - 2026-09-08
### 👑 이광수식 지수 역행 주도주(Relative Strength) 스캐너 엔진 및 대시보드 구축
- **이광수 대표의 주도주 발굴 원칙 시스템화 (`mystock/scanner.py`)**:
  - "물이 빠져야 진짜 실력자가 드러난다": 지수 급락(-1.0% 이상) 또는 하락 구간에서 버티거나 **양봉 마감**하는 종목 포착
  - **이광수식 4대 평가 지표 & 종합 RS 점수(0~100점)** 산출:
    1. **지수 대비 상대 강도 (RS Spread)**: 지수 대비 초과 수익률 (%p)
    2. **양봉 마감 강도 (Candle)**: 하락 압력을 이겨낸 시가 위 반등 강도
    3. **거래량 폭발 (Volume Surge)**: 20거래일 이동평균 대비 거래량 급증율 (스마트 머니 유입)
    4. **AVWAP 수급 지지**: 세력 평단가(AVWAP) 상단 안착 여부
  - **등급 체계**: `🌟 S (85점 이상)`, `🟢 A (70~84점)`, `🟡 B (55~69점)`, `⚪ C/D (관망)`
  - **기계적 손절선(-10%) 및 투자 노트(Trading Plan) 자동 생성**: 5종목 이내 압축 투자 및 매수가 대비 -10% 손절 라인 자동 산출
- **시장 대표 주도주 유니버스 프리셋 (`mystock/universe.py`)**:
  - 저유동성 잡주/품절주를 배제하고 산업 메가트렌드(반도체/AI, 전력기기/인프라, 방산, 조선, 2차전지, 바이오, 밸류업 금융 등)에 부합하는 우량주 유니버스 구축
  - 국내 대표 우량주 (40종목), 미국 대표 빅테크 (17종목), 글로벌 통합 유니버스 (57종목)
- **CLI 명령어 확장 (`main.py`)**:
  - `-l`, `--leader-scan`: 주도주 스캐너 실행
  - `--universe`: `korea`, `us`, `all` 유니버스 선택
  - `-g`, `--group`: 기존 `watchlist.json` 관심 그룹 대상 스캔
  - `--min-score`: 최소 주도주 점수 필터링
  - `--bench`: 벤치마크 지수 수동 지정 (`^KS11`, `^KQ11`, `SPY`, `QQQ`)
- **웹 대시보드 신규 탭 추가 (`app.py`)**:
  - **`👑 이광수식 주도주 스캐너`** 탭 추가
  - KOSPI / KOSDAQ / S&P500 / NASDAQ 지수 상태 및 급락 경고 배너 실시간 표시
  - 스캔 결과 테이블 & 클릭 시 즉시 연동되는 **사이드 피크(Side Peek) 차트 패널**
  - **투자 노트(Trading Plan) 원클릭 확인 및 복사 기능**
  - 발굴된 주도주의 **원클릭 관심종목 등록** 기능
- **단위 테스트 추가 (`tests/test_scanner.py`)**:
  - 지수 하락 시 상대 강도 산출, 역행 양봉 감지, 기계적 손절가 계산에 대한 단위 테스트 작성 및 18개 전체 테스트 통과
